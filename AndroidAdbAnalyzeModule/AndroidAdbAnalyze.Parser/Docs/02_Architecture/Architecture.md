# AndroidAdbAnalyze.Parser 아키텍처

## 문서 개요

본 문서는 `AndroidAdbAnalyze.Parser` 라이브러리의 전체 아키텍처 구조, 핵심 컴포넌트, 데이터 흐름, 그리고 설계 원칙을 설명합니다.
...
아래 다이어그램은 `AndroidAdbAnalyze.Parser`의 핵심 클래스 및 인터페이스 간의 주요 관계를 보여줍니다.
...
classDiagram
    ...
    class AdbLogParser {
        +ParseAsync(string, LogParsingOptions) Task~ParsingResult~
    }

    class LogParsingOptions {
    ...
    class ISectionSplitter {
        <<interface>>
        +SplitAsync(string, IEnumerable~SectionDefinition~) IReadOnlyDictionary
    }

    class ILineParser {
        <<interface>>
        +Parse(string, ParsingContext) ParsedLogEntry
    }
    ...
    class ILogEventRepository {
        <<interface>>
        +SaveEventsAsync(IEnumerable~NormalizedLogEvent~) Task~int~
        +GetEventsByTimeRangeAsync(DateTime, DateTime, string) Task~IEnumerable~
    }
    
    AdbLogParser --|> ILogParser : implements
    AdbLogParser ..> LogParsingOptions : uses
    AdbLogParser ..> ParsingResult : creates
    AdbLogParser ..> ISectionSplitter : uses
    AdbLogParser ..> ILineParser : uses
    AdbLogParser ..> TimestampNormalizer : uses
    
    ParsingResult "1" -- "0..*" NormalizedLogEvent : contains

    ILogParser ..> ILogEventRepository : (client may use)
...
├─────────────────────────────────────────────────────────┤
│                                                          │
│  AdbLogParserEndToEndTests.cs      (통합 테스트)           │
│   ├─ 7가지 로그 타입별 통합 테스트                         │
│   └─ 에러 케이스 테스트 (설정 오류 등)                     │
│                                                          │
│  ActivityLogParserTests.cs         (개별 로직 테스트)            │
│  MediaCameraLogParserTests.cs      (개별 로직 테스트)            │
│  MediaMetricsLogParserTests.cs     (개별 로직 테스트)            │
│                                                          │
│  TestData/                                               │
...
- [API 사용 가이드](../03_Usage_Guides/API_Usage_Guide.md)
- [설정 파일 작성 가이드](../03_Usage_Guides/Configuration_Guide.md)
- [개발 계획](../01_Introduction/DevelopmentPlan.md)
- [개발 가이드라인](../04_Development_Guidelines/AI_Development_Guidelines.md)
