...
foreach (var session in result.Sessions)
{
    ...
    Console.WriteLine($"탐지 점수: {session.SessionCompletenessScore:P0}");
...
foreach (var capture in result.CaptureEvents)
{
    ...
    Console.WriteLine($"탐지 점수: {capture.SessionCompletenessScore:P0}");
...
if (result.Statistics != null)
{
    ...
    Console.WriteLine($"평균 탐지 점수: {result.Statistics.AverageSessionCompletenessScore:P0}");
}
```
...
var highConfidenceSessions = result.Sessions
    .Where(s => s.SessionCompletenessScore >= 0.8)
    .ToList();
...
public sealed class CameraCaptureEvent
{
    ...
    public Guid KeyArtifactId { get; init; }
    public IReadOnlyList<Guid> SupportingArtifactIds { get; init; }
    public bool IsEstimated { get; init; }
    public double SessionCompletenessScore { get; init; }
    public IReadOnlyList<string> ArtifactTypes { get; init; }
    public IReadOnlyList<Guid> SourceEventIds { get; init; }
...
// 증거 타입별 그룹화
var groupedByArtifact = result.CaptureEvents
    .GroupBy(c => string.Join(", ", c.ArtifactTypes.OrderBy(e => e)))
    .Select(g => new { Artifact = g.Key, Count = g.Count() })
...
#### **문제 2: 촬영이 감지되지 않음**
```
원인: 핵심 아티팩트 이벤트 (DATABASE_INSERT, MEDIA_EXTRACTOR 등) 부재
해결:
