...
flowchart TB
    subgraph ext["External System"]
        A["Anlaysis Result Report"]
    end

    subgraph analysis["AndroidAdbAnalyze.Analysis"]
        B["IAnalysisOrchestrator"]
        C["AnalysisResult"]
        D["IReportGenerator"]
    end

    subgraph parser["AndroidAdbAnalyze.Parser"]
        E["AdbLogParser"]
    end
    ...
    %% 글자색 검정 유지 + 테두리 명시
    style A fill:#e1f5ff,color:#000000,stroke:#66b3ff
    style B fill:#ffe1e1,color:#000000,stroke:#ff7f7f
    style D fill:#ffe1e1,color:#000000,stroke:#ff7f7f
    style E fill:#e1ffe1,color:#000000,stroke:#7fff7f
    style C fill:#fff4e1,color:#000000,stroke:#ffc266
```
...
sequenceDiagram
    ...
    loop For Each Session
        ...
        CaptureDetector->>CaptureDetector: FindKeyArtifact()
...
#### **알고리즘: Artifact-based Detection with App-specific Strategies**
...
flowchart TD
    ...
    C --> F[Key Artifact:<br/>VIBRATION_EVENT<br/>hapticType=50061]
    D --> G[Key Artifact:<br/>VIBRATION_EVENT<br/>usage=TOUCH]
    E --> H[Key Artifact:<br/>DATABASE_INSERT<br/>MEDIA_EXTRACTOR<br/>SILENT_CAMERA_CAPTURE]
  
    E --> I[Conditional Key:<br/>VIBRATION_EVENT hapticType=50061<br/>PLAYER_EVENT + PostProcessService<br/>URI_PERMISSION_GRANT temp]
  
    F --> J[Collect Supporting Artifact<br/>±30초 Time Window]
    G --> J
    H --> J
    I --> J
...
2. **Key Artifact**: 핵심 아티팩트 (확정 또는 조건부)
3. **Supporting Artifact**: 보조 아티팩트 (±30초 윈도우)
4. **Detection Score Calculation**: 아티팩트 타입별 가중치 합산
...
```mermaid
flowchart TD
    A[Artifact Events[]] --> B[Extract EventTypes]
...
classDiagram
    class CameraCaptureEvent {
        ...
        +string? FileUri
        +Guid KeyArtifactId
        +IReadOnlyList~Guid~ SupportingArtifactIds
        +bool IsEstimated
        +double ConfidenceScore
...
| **Artifact-based Detection (Strategy)**       | 촬영 감지            | O(n × m)  |
| **Weighted Sum with Type Dedup**              | 탐지 점수 계산       | O(n)       |
...
sequenceDiagram
    ...
    loop 각 세션
        ...
        Strategy->>Strategy: FindKeyArtifact()
        Strategy->>Strategy: CollectSupportingArtifact()
...
flowchart TD
    ...
    MERGE --> CALC_SESSION_CONF[Session Completeness Score<br/>Calculation]
    ...
    KT_STRATEGY --> FIND_PRIMARY[Find Key Artifact<br/>확정 또는 조건부]
    TG_STRATEGY --> FIND_PRIMARY
    BASE_STRATEGY --> FIND_PRIMARY
  
    FIND_PRIMARY --> COLLECT_SUPPORT[Collect Supporting Artifact<br/>±30초 Time Window]
