...
Key Artifact Types (확정 핵심 아티팩트):
  - DATABASE_INSERT
  - MEDIA_EXTRACTOR
  - SILENT_CAMERA_CAPTURE

Conditional Key Artifact Types (조건부 핵심 아티팩트):
...
Supporting Artifact Types (보조 아티팩트):
  - AUDIO_TRACK
  - SHUTTER_SOUND
...
Conditional Key Artifact Types:
...
2. **세션 기반 추정 로직**
   - 핵심 아티팩트 없이도 세션 내 패턴으로 추정
   - `IsEstimated = true`, 낮은 탐지 점수
