...
/// <summary>
/// Sample 9 (9차 샘플) Ground Truth 검증 테스트
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-10-17 16:40:00 ~ 16:52:59):
/// 
/// 기본 카메라 (PID 12308):
/// - 16:40:58-16:41:04 device 20 (촬영 없음)
/// - 16:42:20-16:42:31 device 20 (촬영 1개)
/// 
/// 카카오톡 (taskRootPackage=com.kakao.talk, PID 12308):
/// - 16:43:18-16:43:23 device 20 (촬영 없음)
/// - 16:45:44-16:45:53 device 20 (촬영 1개)
/// 
/// 텔레그램 (PID 464):
/// - 16:46:39-16:46:52 device 0 (촬영 없음)
/// - 16:48:03-16:48:22 device 0 (촬영 1개)
/// 
/// 무음 카메라 (PID 4119):
/// - 16:51:23-16:51:29 device 0 (촬영 없음)
/// - 16:51:49-16:52:00 device 0 (촬영 1개)
/// 
...
/// </remarks>
public sealed class Sample9GroundTruthTests : IAsyncLifetime
{
    ...
    // 샘플 디렉토리 경로
    private const string SampleDirectoryName = "9차 샘플_25_10_17";
    ...
    public Sample9GroundTruthTests(ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("=== Sample 9 (9차 샘플) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (16:42:26)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoTalkCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 1개 있어야 함 (16:45:49)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 1개 있어야 함 (16:48:13)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (16:51:55)");
    ...
    public async Task Should_Match_GroundTruth_SilentCamera_CaptureScore()
    {
        ...
        _output.WriteLine($"ℹ️  Sample 9에서는 VIBRATION_EVENT가 탐지됨 (Sample 6, 7, 8과 동일, Sample 5와 다름)");
        _output.WriteLine("════════════════════════════════════════════════════════════");
    ...
    public async Task Should_DetectCapture_WithExpectedTimestamps()
    {
        ...
        {
            new DateTime(2025, 10, 17, 16, 42, 26),  // 기본 카메라
            new DateTime(2025, 10, 17, 16, 45, 49),  // 카카오톡
            new DateTime(2025, 10, 17, 16, 48, 13),  // 텔레그램
            new DateTime(2025, 10, 17, 16, 51, 55)   // 무음 카메라
        };
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Sample9GroundTruthTests>();
