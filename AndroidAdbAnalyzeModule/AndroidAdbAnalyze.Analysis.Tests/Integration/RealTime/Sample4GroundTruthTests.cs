...
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-10-12 16:07:00 ~ 16:25:00):
/// 
/// 기본 카메라 (PID 24118):
/// - 16:07:01-16:07:05 device 20 (촬영 없음)
/// - 16:07:48-16:07:53 device 20 (촬영 없음)
/// - 16:08:37-16:08:47 device 20 (촬영 1개)
/// 
/// 카카오톡 (taskRootPackage=com.kakao.talk, PID 24118):
/// - 16:12:02-16:12:07 device 20 (촬영 없음)
/// - 16:15:44-16:15:53 device 20 (촬영 1개)
/// - 16:16:56-16:17:05 device 20 (촬영 1개)
/// 
/// 텔레그램 (PID 1329):
/// - 16:19:38-16:19:49 device 0 (후면, 촬영 없음)
/// - 16:20:50-16:21:08 device 0 (후면, 촬영 1개)
/// - 16:22:04-16:22:18 device 0 (후면, 촬영 1개)
/// - 16:23:06-16:23:11 device 0 (후면, 촬영 없음 - 앨범 전송)
/// 
/// 무음 카메라 (PID 6034):
/// - 16:23:48-16:23:54 device 0 (후면, 촬영 없음)
/// - 16:24:19-16:24:30 device 0 (후면, 촬영 1개)
/// 
/// 참고: 이 Samsung Galaxy 기기에서 device 0과 device 20은 모두 후면 카메라입니다.
/// 
/// Ground Truth (실제 로그 기반):
/// - 총 세션: 12개 (usagestats + media_camera 병합됨)
/// - 총 촬영: 6개 (기본 카메라 1 + 카카오톡 2 + 텔레그램 2 + 무음 카메라 1)
/// 
/// 참고: 모듈은 시작+종료 시간 차이가 2초 이내인 usagestats와 media_camera를 
///       같은 카메라 사용으로 판단하여 자동 병합합니다.
/// </remarks>
public sealed class Sample4GroundTruthTests : IAsyncLifetime
{
    ...
    // 샘플 디렉토리 경로
    private const string SampleDirectoryName = "4차 샘플_25_10_12";
    ...
    {
        _output.WriteLine("=== Sample 4 (4차 샘플) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_TotalSessions()
    {
        ...
        result.Sessions.Should().HaveCount(ExpectedTotalSessions,
            "실제 로그에 따르면 12개의 카메라 세션이 있어야 함 (usagestats+media_camera 병합됨)");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (16:08:42)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 2개 있어야 함 (16:15:48, 16:17:00)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 2개 있어야 함 (16:20:59, 16:22:13)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (16:24:24)");
    ...
    public async Task Should_DetectCapture_WithExpectedTimestamps()
    {
        ...
        {
            ["camera"] = new[] { new DateTime(2025, 10, 12, 16, 8, 42) },
            ["kakao"] = new[] 
            { 
            ...
            ["telegram"] = new[] 
            { 
                new DateTime(2025, 10, 12, 16, 20, 59),
                new DateTime(2025, 10, 12, 16, 22, 13)
            },
            ["Silent"] = new[] { new DateTime(2025, 10, 12, 16, 24, 24) }
        };
    ...
    public async Task Should_Match_GroundTruth_KakaoTalk_CaptureScore()
    {
        ...
        foreach (var (capture, index) in capturesWithScore.Select((c, i) => (c, i)))
        {
            ...
            capture.ArtifactTypes.Should().Contain("CAMERA_ACTIVITY_REFRESH",
                "CAMERA_ACTIVITY_REFRESH 이벤트가 탐지되어야 함");
            capture.ArtifactTypes.Should().Contain("MEDIA_EXTRACTOR",
                "MEDIA_EXTRACTOR 이벤트가 탐지되어야 함");
            capture.ArtifactTypes.Should().Contain("PLAYER_RELEASED",
                "PLAYER_RELEASED 이벤트가 탐지되어야 함");
    ...
    public async Task Should_Match_GroundTruth_Telegram_CaptureScore()
    {
        // Arrange
        var options = CreateAnalysisOptions();
        var expectedScoreCapture1 = 0.75; // VIBRATION(0.4) + CAM_ACTIVITY(0.15) + MEDIA_EXTRACTOR(0.2)
        var expectedScoreCapture2 = 1.00; // Capture1 아티팩트 + PLAYER_CREATED(0.25) -> MaxConfidence
        var tolerance = 0.1;
        ...
        _output.WriteLine($"\nℹ️  특징: VIBRATION(0.4) + CAM_ACTIVITY(0.15) + MEDIA_EXTRACTOR(0.2)");
        _output.WriteLine("════════════════════════════════════════════════════════════");
        ...
        capture1.ArtifactTypes.Should().Contain("VIBRATION_EVENT", "촬영 #1은 TOUCH 진동이 탐지되어야 함");
        capture1.ArtifactTypes.Should().Contain("CAMERA_ACTIVITY_REFRESH", "촬영 #1은 Activity 갱신 이벤트가 탐지되어야 함");
        capture1.ArtifactTypes.Should().Contain("MEDIA_EXTRACTOR", "촬영 #1은 미디어 추출기 이벤트가 탐지되어야 함");
        capture1.ArtifactTypes.Should().NotContain("PLAYER_CREATED", "촬영 #1은 PLAYER_CREATED가 없어야 함");
        capture1.ArtifactTypes.Should().NotContain("DATABASE_INSERT", "텔레그램은 DATABASE_INSERT가 없어야 함");

        // 두 번째 촬영 검증 (1.00점, PLAYER_CREATED 추가)
        ...
        _output.WriteLine($"\nℹ️  특징: Capture1 아티팩트 + PLAYER_CREATED(0.25) → MaxConfidence 캡핑");
        _output.WriteLine("════════════════════════════════════════════════════════════");
        
        capture2.CaptureDetectionScore.Should().BeInRange(expectedScoreCapture2 - tolerance, expectedScoreCapture2 + tolerance,
            $"텔레그램 촬영 #2 점수는 {expectedScoreCapture2:F2} ± {tolerance:F2} 범위여야 함 (MaxConfidence 캡핑)");
        
        capture2.ArtifactTypes.Should().Contain("VIBRATION_EVENT", "촬영 #2는 TOUCH 진동이 탐지되어야 함");
        capture2.ArtifactTypes.Should().Contain("CAMERA_ACTIVITY_REFRESH", "촬영 #2는 Activity 갱신 이벤트가 탐지되어야 함");
        capture2.ArtifactTypes.Should().Contain("MEDIA_EXTRACTOR", "촬영 #2는 미디어 추출기 이벤트가 탐지되어야 함");
        capture2.ArtifactTypes.Should().Contain("PLAYER_CREATED", "Ground Truth에 따라 촬영 #2는 예상치 못하게 PLAYER_CREATED가 탐지되어야 함");
        capture2.ArtifactTypes.Should().NotContain("DATABASE_INSERT", "텔레그램은 DATABASE_INSERT가 없어야 함");
    ...
    public async Task Should_Verify_AllCaptures_MeetMinimumThreshold()
    {
        ...
        // Assert
        _output.WriteLine($"=== 최소 임계값 검증 (MinConfidence: {minThreshold}) ===");
        
        foreach (var capture in result.CaptureEvents)
        {
            _output.WriteLine($"\n{capture.PackageName}:");
            _output.WriteLine($"  촬영 시각: {capture.CaptureTime:HH:mm:ss}");
            ...
            capture.CaptureDetectionScore.Should().BeGreaterThanOrEqualTo(minThreshold,
                $"{capture.PackageName} 촬영 점수는 최소 임계값({minThreshold}) 이상이어야 함");
        }
    ...
    public async Task Should_Verify_CaptureScore_Distribution()
    {
        ...
        {
            var scores = group.Select(c => c.CaptureDetectionScore).ToList();
            var avgScore = scores.Average();
            var minScore = scores.Min();
            var maxScore = scores.Max();

            _output.WriteLine($"\n{group.Key}:");
            _output.WriteLine($"  촬영 수: {scores.Count}");
            _output.WriteLine($"  평균 점수: {avgScore:F2}");
            _output.WriteLine($"  최소 점수: {minScore:F2}");
            _output.WriteLine($"  최대 점수: {maxScore:F2}");
            _output.WriteLine($"  점수 목록: {string.Join(", ", scores.Select(s => s.ToString("F2")))}");
        }
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Sample4GroundTruthTests>();
