...
/// <summary>
/// Sample 6 (6차 샘플) Ground Truth 검증 테스트
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-10-16 16:34:00 ~ 16:48:59):
/// 
/// 기본 카메라 (PID 23116):
/// - 16:34:53-16:34:58 device 20 (촬영 없음)
/// - 16:36:10-16:36:20 device 20 (촬영 1개)
/// 
/// 카카오톡 (taskRootPackage=com.kakao.talk, PID 23116):
/// - 16:37:23-16:37:28 device 20 (촬영 없음)
/// - 16:38:42-16:38:51 device 20 (촬영 1개)
/// 
/// 기본 카메라 (PID 30106) - 전면/후면 전환:
/// - 16:44:52-16:45:02 device 20 → device 21 (후면에서 전면)
/// - 16:45:02-16:45:04 device 21 (전면)
/// - 16:45:04-16:45:11 device 20 (다시 후면)
/// 
/// 텔레그램:
/// - 16:45:42-16:45:49 device 0 (후면, 촬영 없음)
/// - 16:46:42-16:46:56 device 0 (후면, 촬영 1개)
/// 
/// 무음 카메라:
/// - 16:47:29-16:47:34 device 0 (후면, 촬영 없음)
/// - 16:48:15-16:48:26 device 0 (후면, 촬영 1개)
/// 
/// 참고: 이 Samsung Galaxy 기기에서 device 0과 device 20은 모두 후면 카메라, device 21은 전면 카메라입니다.
/// 
/// Ground Truth (실제 로그 기반):
/// - 총 세션: 11개 (기본 카메라 2 + 카카오톡 2 + 전면/후면 전환 3 + 텔레그램 2 + 무음 카메라 2)
/// - 총 촬영: 4개 (기본 카메라 1 + 카카오톡 1 + 텔레그램 1 + 무음 카메라 1)
/// 
/// 추가 참고:
/// - 전면/후면 카메라 전환 시 3개의 별도 CONNECT/DISCONNECT 이벤트 발생 (정상 동작)
/// - 카카오톡에서 실행한 카메라는 물리적으로 com.sec.android.app.camera이지만, 
///   usagestats.log의 taskRootPackage=com.kakao.talk으로 카카오톡 세션으로 분류됨
/// </remarks>
public sealed class Sample6GroundTruthTests : IAsyncLifetime
{
    ...
    // 샘플 디렉토리 경로
    private const string SampleDirectoryName = "6차 샘플_25_10_16";
    
    // 분석 시간 범위 (실제 로그 기준)
    private readonly DateTime _startTime = new(2025, 10, 16, 16, 34, 0);
    private readonly DateTime _endTime = new(2025, 10, 16, 16, 48, 59);

    // 아티팩트 가중치 (TestConstants에서 참조)
    private static readonly IReadOnlyDictionary<string, double> Weights = ArtifactWeights.Standard;

    public Sample6GroundTruthTests(ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("=== Sample 6 (6차 샘플) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_TotalSessions()
    {
        ...
        result.Sessions.Should().HaveCount(ExpectedTotalSessions,
            "실제 로그에 따르면 11개의 카메라 세션이 있어야 함 (기본 카메라 2 + 카카오톡 2 + 전면/후면 전환 3 + 텔레그램 2 + 무음 카메라 2)");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (16:36:12, 카카오톡 제외)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoTalkCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 1개 있어야 함 (16:38:43)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 1개 있어야 함 (16:46:44)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        var silentCameraCaptures = result.CaptureEvents
            .Count(c => c.PackageName?.Contains("SilentCamera", StringComparison.OrdinalIgnoreCase) == true ||
                       c.PackageName?.Contains("peace", StringComparison.OrdinalIgnoreCase) == true);

        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (16:48:17)");
    ...
    public async Task Should_HaveValidSessionData()
    {
        ...
        _output.WriteLine($"✓ 모든 세션 데이터 유효성 검증 통과 ({result.Sessions.Count}개 세션)");
    }
    ...
    public async Task Should_HaveValidCaptureData()
    {
        ...
        foreach (var capture in result.CaptureEvents)
        {
            ...
            capture.ParentSessionId.Should().NotBeEmpty();
            capture.CaptureId.Should().NotBeEmpty();
        }

        _output.WriteLine($"✓ 모든 촬영 데이터 유효성 검증 통과 ({result.CaptureEvents.Count}개 촬영)");
    }
    ...
    public async Task Should_DetectCapture_WithExpectedTimestamps()
    {
        ...
        {
            new DateTime(2025, 10, 16, 16, 36, 12), // 기본 카메라
            new DateTime(2025, 10, 16, 16, 38, 43), // 카카오톡
            new DateTime(2025, 10, 16, 16, 46, 44), // 텔레그램
            new DateTime(2025, 10, 16, 16, 48, 17)  // 무음 카메라
        };
    ...
    public async Task Generate_GroundTruth_Document()
    {
        ...
        var sampleInfo = new ArtifactWeights.SampleInfo(
            SampleNumber: 6,
            SampleName: "6차 샘플",
            TestDate: new DateTime(2025, 10, 16),
            TimeRange: (_startTime, _endTime),
            Description: "전면/후면 카메라 전환 시나리오 포함 (총 4회 촬영)"
        );
        ...
        gtDocument.Should().Contain("# Sample 6", "헤더가 있어야 함");
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Sample6GroundTruthTests>();
