...
/// <summary>
/// Sample 2 (2차 샘플) Ground Truth 검증 테스트
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-10-06 22:46:00 ~ 22:59:00):
/// 
/// 기본 카메라 (PID 14297):
/// - 22:46:43-22:46:47 device 20 (촬영 없음)
/// - 22:47:40-22:47:51 device 20 (촬영 1개)
/// 
/// 카카오톡 (taskRootPackage=com.kakao.talk, PID 14297):
/// - 22:48:51-22:48:56 device 20 (촬영 없음)
/// - 22:49:52-22:50:01 device 20 (촬영 1개)
/// - 22:50:54-22:51:03 device 20 (촬영 1개)
/// 
/// 텔레그램 (PID 20586):
/// - 22:53:25-22:53:36 device 0 (후면, 촬영 없음)
/// - 22:54:29-22:54:47 device 0 (후면, 촬영 1개)
/// - 22:55:24-22:55:38 device 0 (후면, 촬영 1개)
/// - 22:56:52-22:56:57 device 0 (후면, 촬영 없음 - 앨범 전송)
/// 
/// 무음 카메라 (PID 23095):
/// - 22:57:37-22:57:43 device 0 (후면, 촬영 없음)
/// - 22:58:22-22:58:33 device 0 (후면, 촬영 1개)
/// 
...
/// 
/// 참고:
/// - 데이터 시트에는 카카오톡 앨범 전송(22:52:32)이 별도 항목으로 기재되어 있으나,
///   media_camera.log에는 해당 시간에 카메라 세션이 없음 (앨범 전송만 발생)
/// - 모듈은 시작+종료 시간 차이가 2초 이내인 usagestats와 media_camera를 자동 병합
/// </remarks>
public sealed class Sample2GroundTruthTests : IAsyncLifetime
{
    ...
    public Sample2GroundTruthTests(ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("=== Sample 2 (2차 샘플) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_TotalSessions()
    {
        ...
        // 🔍 디버깅: CAMERA_ACTIVITY_REFRESH 이벤트 확인
        var cameraActivityRefreshEvents = _parsedEvents!
            .Where(e => e.EventType == "CAMERA_ACTIVITY_REFRESH")
            .ToList();
        _output.WriteLine($"🔍 DEBUG: CAMERA_ACTIVITY_REFRESH 이벤트 수: {cameraActivityRefreshEvents.Count}");
        foreach (var evt in cameraActivityRefreshEvents.Take(5))
        {
            var package = evt.Attributes.TryGetValue("package", out var pkg) ? pkg.ToString() : "N/A";
            _output.WriteLine($"  - Time={evt.Timestamp:HH:mm:ss.fff}, Package={package}, EventType={evt.EventType}");
        }
        ...
        result.Sessions.Should().HaveCount(ExpectedTotalSessions,
            "실제 로그에 따르면 11개의 카메라 세션이 있어야 함 (usagestats+media_camera 병합됨)");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (22:47:45)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 2개 있어야 함 (22:49:56, 22:50:58)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 2개 있어야 함 (22:54:38, 22:55:33)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (22:58:27)");
    ...
    public async Task Should_DetectCapture_WithExpectedTimestamps()
    {
        ...
        {
            ["camera"] = new[] { new DateTime(2025, 10, 6, 22, 47, 45) },
            ["kakao"] = new[] 
            { 
            ...
            ["Silent"] = new[] { new DateTime(2025, 10, 6, 22, 58, 27) }
        };
    ...
    public async Task Should_Match_GroundTruth_DefaultCamera_CaptureScore()
    {
        // Arrange
        var options = CreateAnalysisOptions();
        // 실제 계산값: DATABASE_INSERT(0.5) + VIBRATION(0.4) + FOREGROUND_SERVICE(0.3) + PLAYER_CREATED(0.25) + MEDIA_EXTRACTOR(0.2) = 1.65 (PLAYER_RELEASED 미탐지 가정)
        // MaxConfidence 캡핑 제거 후: 1.80 (실제 로그 기준)
        ...
        _output.WriteLine($"\n✅ 기본 카메라 촬영 점수 검증 완료");
        _output.WriteLine($"💡 CAMERA_ACTIVITY_REFRESH는 activity.log에 [ModeId] 패턴만 있어 미탐지 (파서가 [Min/Max]만 인식)");
    }
    ...
    public async Task Should_Match_GroundTruth_KakaoTalk_CaptureScore()
    {
        ...
        foreach (var (capture, index) in capturesWithScore.Select((c, i) => (c, i)))
        {
            ...
            capture.ArtifactTypes.Should().NotContain("DATABASE_INSERT",
                "카카오톡은 Camera2+CUA 특성상 DATABASE_INSERT가 없어야 함");
            capture.ArtifactTypes.Should().Contain("VIBRATION_EVENT",
                "카카오톡 촬영 시 진동 이벤트가 탐지되어야 함");
            capture.ArtifactTypes.Should().Contain("PLAYER_EVENT",
                "카카오톡도 실제로는 PLAYER_EVENT 발생 (로그에서 확인)");
            capture.ArtifactTypes.Should().Contain("PLAYER_CREATED",
                "촬영 사운드 재생을 위한 플레이어 생성 이벤트가 탐지되어야 함");
            capture.ArtifactTypes.Should().Contain("PLAYER_RELEASED",
                "사용한 플레이어 해제 이벤트가 탐지되어야 함");
    ...
    public async Task Should_Match_GroundTruth_Telegram_CaptureScore()
    {
        ...
        // 촬영 #1: 0.75 (CAMERA_ACTIVITY_REFRESH 포함)
        var capture1 = capturesWithScore[0];
        var expectedScore1 = 0.75;
        ...
        _output.WriteLine($"🎯 예상 점수: {expectedScore1:F2} (±{tolerance:F2})");
        ...
        capture1.CaptureDetectionScore.Should().BeInRange(expectedScore1 - tolerance, expectedScore1 + tolerance,
            $"텔레그램 촬영 #1 점수는 {expectedScore1:F2} ± {tolerance:F2} 범위여야 함");

        capture1.ArtifactTypes.Should().Contain("VIBRATION_EVENT",
            "텔레그램 특유의 TOUCH 진동이 탐지되어야 함");
        capture1.ArtifactTypes.Should().Contain("MEDIA_EXTRACTOR",
            "미디어 추출기 이벤트가 탐지되어야 함");
        capture1.ArtifactTypes.Should().NotContain("DATABASE_INSERT",
            "텔레그램은 DATABASE_INSERT가 없어야 함");
        capture1.ArtifactTypes.Should().NotContain("PLAYER_EVENT",
            "텔레그램은 셔터음이 없어야 함");
        capture1.ArtifactTypes.Should().NotContain("PLAYER_CREATED",
            "텔레그램 촬영 #1은 PLAYER_CREATED가 없어야 함");

        // 촬영 #2: 1.00 (CAMERA_ACTIVITY_REFRESH 포함, MaxConfidence 캡핑)
        var capture2 = capturesWithScore[1];
        var expectedScore2 = 1.00;
        ...
        _output.WriteLine($"🎯 예상 점수: {expectedScore2:F2} (±{tolerance:F2})");
        ...
        _output.WriteLine($"\nℹ️  특징: 예상치 못하게 PLAYER_CREATED가 탐지됨 (Sample 3 촬영 #2와 유사)");
        _output.WriteLine("════════════════════════════════════════════════════════════");

        capture2.CaptureDetectionScore.Should().BeInRange(expectedScore2 - tolerance, expectedScore2 + tolerance,
            $"텔레그램 촬영 #2 점수는 {expectedScore2:F2} ± {tolerance:F2} 범위여야 함 " +
            "(VIBRATION 0.4 + MEDIA_EXTRACTOR 0.2 + PLAYER_CREATED 0.25 + CAMERA_ACTIVITY_REFRESH 0.15 = 1.00, MaxConfidence 캡핑)");
        ...
        capture2.ArtifactTypes.Should().Contain("PLAYER_CREATED",
            "텔레그램 촬영 #2는 예상치 못하게 PLAYER_CREATED가 탐지되어야 함");
        capture2.ArtifactTypes.Should().NotContain("DATABASE_INSERT",
            "텔레그램은 DATABASE_INSERT가 없어야 함");
        capture2.ArtifactTypes.Should().NotContain("PLAYER_EVENT",
            "텔레그램은 셔터음이 없어야 함");
    ...
    public async Task Should_Match_GroundTruth_SilentCamera_CaptureScore()
    {
        ...
        var expectedScore = 1.05;
        var tolerance = 0.10; // 실제 점수 범위: 0.95~1.15
    ...
    public async Task Should_Verify_AllCaptures_MeetMinimumThreshold()
    {
        ...
        {
            _output.WriteLine($"\n{capture.PackageName}:");
            _output.WriteLine($"  시각: {capture.CaptureTime:HH:mm:ss}");
            _output.WriteLine($"  점수: {capture.CaptureDetectionScore:F2}");
            _output.WriteLine($"  아티팩트 수: {capture.ArtifactTypes.Count}");

            capture.CaptureDetectionScore.Should().BeGreaterThanOrEqualTo(minThreshold,
                $"{capture.PackageName}의 촬영 점수는 최소 임계값 {minThreshold} 이상이어야 함");
        }

        _output.WriteLine($"\n✅ 모든 촬영이 최소 임계값 {minThreshold}을 충족함");
    }
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Sample2GroundTruthTests>();
