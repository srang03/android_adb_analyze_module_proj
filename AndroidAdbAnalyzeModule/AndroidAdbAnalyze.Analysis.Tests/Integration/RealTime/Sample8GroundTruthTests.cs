...
/// <summary>
/// Sample 8 (8차 샘플) Ground Truth 검증 테스트
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-10-17 16:00:00 ~ 16:07:59):
/// 
/// 기본 카메라 (PID 12308):
/// - 16:00:52-16:00:57 device 20 (촬영 없음)
/// - 16:01:27-16:01:37 device 20 (촬영 1개)
/// 
/// 카카오톡 (taskRootPackage=com.kakao.talk, PID 12308):
/// - 16:02:37-16:02:42 device 20 (촬영 없음)
/// - 16:03:29-16:03:39 device 20 (촬영 1개)
/// 
/// 텔레그램 (PID 11737):
/// - 16:04:11-16:04:22 device 0 (촬영 없음)
/// - 16:05:24-16:05:43 device 0 (촬영 1개)
/// 
/// 무음 카메라 (PID 19007):
/// - 16:06:13-16:06:18 device 0 (촬영 없음)
/// - 16:07:15-16:07:26 device 0 (촬영 1개)
/// 
...
/// </remarks>
public sealed class Sample8GroundTruthTests : IAsyncLifetime
{
    ...
    // 샘플 디렉토리 경로
    private const string SampleDirectoryName = "8차 샘플_25_10_17";
    
    // 분석 시간 범위 (실제 로그 기준)
    private readonly DateTime _startTime = new(2025, 10, 17, 16, 0, 0);
    private readonly DateTime _endTime = new(2025, 10, 17, 16, 7, 59);

    // 아티팩트 가중치 (TestConstants에서 참조)
    private static readonly IReadOnlyDictionary<string, double> Weights = ArtifactWeights.Standard;

    public Sample8GroundTruthTests(ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("=== Sample 8 (8차 샘플) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (16:01:32)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoTalkCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 1개 있어야 함 (16:03:34)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 1개 있어야 함 (16:05:34)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (16:07:21)");
    ...
    public async Task Should_DetectCapture_WithExpectedTimestamps()
    {
        ...
        {
            new DateTime(2025, 10, 17, 16, 1, 32),  // 기본 카메라
            new DateTime(2025, 10, 17, 16, 3, 34),  // 카카오톡
            new DateTime(2025, 10, 17, 16, 5, 34),  // 텔레그램
            new DateTime(2025, 10, 17, 16, 7, 21)   // 무음 카메라
        };
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Sample8GroundTruthTests>();
