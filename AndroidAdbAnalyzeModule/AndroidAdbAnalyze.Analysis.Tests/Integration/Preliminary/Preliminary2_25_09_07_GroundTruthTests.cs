...
/// <summary>
/// Preliminary Test 2 (예비 실험 2차) Ground Truth 검증 테스트
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-09-06 10:10:00 ~ 10:22:00):
/// 
/// 기본 카메라:
/// - 10:10:52-10:10:57 (촬영 없음)
/// - 10:11:25-10:11:35 (촬영 1개, 10:11:30)
/// 
/// 카카오톡:
/// - 10:12:31-10:12:36 (촬영 없음)
/// - 10:13:52-10:14:02 (촬영 1개, 10:13:57)
/// 
/// 텔레그램:
/// - 10:17:05-10:17:10 (촬영 없음)
/// - 10:18:13-10:18:23 (촬영 1개, 10:18:18)
/// 
/// 무음 카메라:
/// - 10:20:58-10:21:03 (촬영 없음)
/// - 10:21:44-10:21:54 (촬영 1개, 10:21:49)
/// 
/// ⚠️ 주의: 데이터 시트에는 2025-09-07로 기록되어 있으나, 실제 로그 파일은 2025-09-06입니다.
/// 
/// Ground Truth (실제 로그 기반):
/// - 총 세션: 8개 (기본 카메라 2 + 카카오톡 2 + 텔레그램 2 + 무음 카메라 2)
/// - 총 촬영: 4개 (기본 카메라 1 + 카카오톡 1 + 텔레그램 1 + 무음 카메라 1)
/// </remarks>
public sealed class Preliminary2_25_09_07_GroundTruthTests : IAsyncLifetime
{
    ...
    // 샘플 디렉토리 경로
    private const string SampleDirectoryName = "예비 실험/예비 실험 2차 25_09_06";
    
    // 분석 시간 범위 (실제 로그 날짜: 2025-09-06, 데이터 시트 날짜: 2025-09-07)
    private readonly DateTime _startTime = new(2025, 9, 6, 10, 10, 0);
    private readonly DateTime _endTime = new(2025, 9, 6, 10, 22, 59);

    public Preliminary2_25_09_07_GroundTruthTests(ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("=== Preliminary Test 2 (예비 실험 2차) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (10:11:30)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 1개 있어야 함 (10:13:57)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 1개 있어야 함 (10:18:18)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (10:21:49)");
    ...
    public async Task Generate_GroundTruth_Document()
    {
        ...
        var sampleInfo = new ArtifactWeights.SampleInfo(
            ...
            SampleName: "예비 실험 2차",
            TestDate: new DateTime(2025, 9, 6),
        ...
        var outputPath = Path.Combine(docDir, "Preliminary2_Ground_Truth.md");
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Preliminary2_25_09_07_GroundTruthTests>();
