...
public sealed class ArtifactFrequencyValidationTests : IAsyncLifetime
{
    ...
    {
        _output.WriteLine("════════════════════════════════════════════════════════════");
        _output.WriteLine("🔬 앱별 아티팩트 출현 빈도 측정 테스트 초기화 (예비 실험 3회)");
        _output.WriteLine("════════════════════════════════════════════════════════════\n");
        
        // 예비 실험 3회 분석
        _allParsedEvents = await ParseAllPreliminaryEventsAsync();
        ...
        _output.WriteLine("✅ 예비 실험 3회 분석 완료\n");
    }
    ...
    /// </summary>
    /// <remarks>
    /// 논문 부록 3, 표 34 "앱별 주요 아티팩트 출현 빈도 (예비 실험 3회)" 데이터 생성
    /// </remarks>
    [Fact]
    public void Measure_ArtifactFrequency_PreliminaryExperiments()
    {
        _output.WriteLine("════════════════════════════════════════════════════════════");
        _output.WriteLine("📊 앱별 아티팩트 출현 빈도 측정 (예비 실험 3회)");
        _output.WriteLine("════════════════════════════════════════════════════════════\n");
        ...
        // 2. 결과 출력 (표 형식)
        _output.WriteLine("─────────────────────────────────────────────────────────────────────");
        _output.WriteLine("[표] 앱별 주요 아티팩트 출현 빈도 (예비 실험 3회)");
        _output.WriteLine("─────────────────────────────────────────────────────────────────────\n");
        ...
        if (databaseInsertFreq[0] >= 1.0 && databaseInsertFreq[3] >= 1.0 && databaseInsertFreq[1] == 0 && databaseInsertFreq[2] == 0)
        {
            _output.WriteLine("  - DATABASE_INSERT: 기본/무음 카메라만 100% 생성 (MediaStore 등록 방식)");
        }
        ...
        if (playerEventFreq[0] >= 1.0 && playerEventFreq[1] >= 1.0 && playerEventFreq[2] >= 1.0 && playerEventFreq[3] == 0)
        {
            _output.WriteLine("  - PLAYER_EVENT: 무음 카메라 제외한 모든 앱에서 100% 출현 (셔터음 재생)");
        }
        
        _output.WriteLine("\n💡 전략적 시사점:");
        _output.WriteLine("  - 앱별 출현 패턴 차이로 인해 Strategy Pattern 기반 차별화 전략 필요");
        _output.WriteLine("  - BasePatternStrategy (기본/무음): DATABASE_INSERT 기반 탐지");
        _output.WriteLine("  - KakaoTalkStrategy: URI_PERMISSION_GRANT 기반 탐지");
        _output.WriteLine("  - TelegramStrategy: VIBRATION_EVENT + MEDIA_EXTRACTOR 조합 탐지");
        _output.WriteLine("  - VIBRATION_EVENT는 모든 앱에서 100% 출현으로 범용 아티팩트로 활용 가능\n");
        
        // 4. JSON 파일로 결과 저장
        var resultPath = Path.Combine(Directory.GetCurrentDirectory(), "preliminary_experiment_artifact_frequency_result.json");
        var result = new
        {
            MeasurementScope = "예비 실험 3회 (12개 촬영)",
        ...
        _output.WriteLine($"   - Ground Truth: 12개 (예비 실험 3회 × 4개 앱)");
        
        if (_totalDetectedCaptures == 12)
        {
            _output.WriteLine($"   ✅ Ground Truth와 일치!");
        }
        else
        {
            _output.WriteLine($"   ⚠️ 차이: {_totalDetectedCaptures - 12}개");
        }
    ...
    /// <summary>
    /// 예비 실험 3회 로그 파싱 (Ground Truth 시간 범위 사용)
    /// </summary>
    private async Task<List<NormalizedLogEvent>> ParseAllPreliminaryEventsAsync()
    {
        ...
        {
            ("예비 실험/예비 실험 1차 25_09_01", new DateTime(2025, 9, 1, 9, 45, 0), new DateTime(2025, 9, 1, 9, 53, 0)),
            ("예비 실험/예비 실험 2차 25_09_06", new DateTime(2025, 9, 6, 10, 10, 0), new DateTime(2025, 9, 6, 10, 22, 59)),
            ("예비 실험/예비 실험 3차 25_09_07", new DateTime(2025, 9, 7, 10, 35, 0), new DateTime(2025, 9, 7, 10, 44, 59))
        };

        foreach (var (sampleDir, startTime, endTime) in preliminarySamples)
        {
            var samplePath = Path.Combine(_sampleLogsPath, sampleDir);
