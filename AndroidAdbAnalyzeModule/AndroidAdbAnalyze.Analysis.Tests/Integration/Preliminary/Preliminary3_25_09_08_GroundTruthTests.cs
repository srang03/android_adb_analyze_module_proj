...
/// <summary>
/// Preliminary Test 3 (예비 실험 3차) Ground Truth 검증 테스트
/// </summary>
/// <remarks>
/// 실제 로그 기반 Ground Truth (2025-09-07 10:35:00 ~ 10:44:00):
/// 
/// 기본 카메라:
/// - 10:35:46-10:35:51 (촬영 없음)
/// - 10:36:29-10:36:39 (촬영 1개, 10:36:34)
/// 
/// 카카오톡:
/// - 10:37:42-10:37:47 (촬영 없음)
/// - 10:39:35-10:39:45 (촬영 1개, 10:39:40)
/// 
/// 텔레그램:
/// - 10:40:31-10:40:36 (촬영 없음)
/// - 10:41:39-10:41:49 (촬영 1개, 10:41:44)
/// 
/// 무음 카메라:
/// - 10:43:06-10:43:11 (촬영 없음)
/// - 10:43:34-10:43:44 (촬영 1개, 10:43:39)
/// 
/// ⚠️ 주의: 데이터 시트에는 2025-09-08로 기록되어 있으나, 실제 로그 파일은 2025-09-07입니다.
/// 
/// Ground Truth (실제 로그 기반):
/// - 총 세션: 8개 (기본 카메라 2 + 카카오톡 2 + 텔레그램 2 + 무음 카메라 2)
/// - 총 촬영: 4개 (기본 카메라 1 + 카카오톡 1 + 텔레그램 1 + 무음 카메라 1)
/// </remarks>
public sealed class Preliminary3_25_09_08_GroundTruthTests : IAsyncLifetime
{
    ...
    // 샘플 디렉토리 경로
    private const string SampleDirectoryName = "예비 실험/예비 실험 3차 25_09_07";
    
    // 분석 시간 범위 (실제 로그 날짜: 2025-09-07, 데이터 시트 날짜: 2025-09-08)
    private readonly DateTime _startTime = new(2025, 9, 7, 10, 35, 0);
    private readonly DateTime _endTime = new(2025, 9, 7, 10, 44, 59);

    public Preliminary3_25_09_08_GroundTruthTests(ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("=== Preliminary Test 3 (예비 실험 3차) Ground Truth 테스트 초기화 ===");
    ...
    public async Task Should_Match_GroundTruth_DefaultCameraCaptures()
    {
        ...
        defaultCameraCaptures.Should().Be(ExpectedDefaultCameraCaptures,
            "데이터 시트에 따르면 기본 카메라 촬영이 1개 있어야 함 (10:36:34)");
    ...
    public async Task Should_Match_GroundTruth_KakaoTalkCaptures()
    {
        ...
        kakaoCaptures.Should().Be(ExpectedKakaoTalkCaptures,
            "데이터 시트에 따르면 카카오톡 촬영이 1개 있어야 함 (10:39:40)");
    ...
    public async Task Should_Match_GroundTruth_TelegramCaptures()
    {
        ...
        telegramCaptures.Should().Be(ExpectedTelegramCaptures,
            "데이터 시트에 따르면 텔레그램 촬영이 1개 있어야 함 (10:41:44)");
    ...
    public async Task Should_Match_GroundTruth_SilentCameraCaptures()
    {
        ...
        silentCameraCaptures.Should().Be(ExpectedSilentCameraCaptures,
            "데이터 시트에 따르면 무음 카메라 촬영이 1개 있어야 함 (10:43:39)");
    ...
    public async Task Generate_GroundTruth_Document()
    {
        ...
        var sampleInfo = new ArtifactWeights.SampleInfo(
            ...
            SampleName: "예비 실험 3차",
            TestDate: new DateTime(2025, 9, 7),
        ...
        var outputPath = Path.Combine(docDir, "Preliminary3_Ground_Truth.md");
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Preliminary3_25_09_08_GroundTruthTests>();
