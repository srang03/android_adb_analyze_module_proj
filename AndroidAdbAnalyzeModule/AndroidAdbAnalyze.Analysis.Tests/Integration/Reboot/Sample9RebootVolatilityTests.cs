...
/// <summary>
/// Sample 9 재부팅 휘발성 테스트: 재부팅 후 로그 분석 성능 검증
/// 
/// Ground Truth (원본 9차 샘플):
/// - 총 세션: 8개
/// - 총 촬영: 4개
///   - 기본 카메라: 1개 (S9-2: 16:42:26)
///   - 카카오톡: 1개 (S9-4: 16:45:49)
///   - 텔레그램: 1개 (S9-6: 16:48:13)
///   - 무음 카메라: 1개 (S9-8: 16:51:55)
/// 
/// 재부팅 특성:
/// - 재부팅 시점: 2025-10-16 18:59:19 (DEVICE_SHUTDOWN)
/// - 재부팅 후 시작: 2025-10-17 10:12:45
/// - 촬영 시간: 2025-10-17 16:40 ~ 16:52 (재부팅 약 6시간 후)
/// - usagestats.log: 3,326 라인 (재부팅 후 6시간 기록 포함)
/// </summary>
public sealed class Sample9RebootVolatilityTests : IAsyncLifetime
{
    ...
    private const string RebootSampleDirectoryName = "재부팅 휘발성/9차 샘플_25_10_17_재부팅";
    
    private readonly DateTime _startTime = new(2025, 10, 17, 16, 40, 0);
    private readonly DateTime _endTime = new(2025, 10, 17, 16, 52, 59);

    // 재부팅 시점 정보
    private readonly DateTime _deviceShutdownTime = new(2025, 10, 16, 18, 59, 19);
    private readonly DateTime _deviceStartupTime = new(2025, 10, 17, 10, 12, 45);

    public Sample9RebootVolatilityTests(ITestOutputHelper output)
    {
        _output = output;
    ...
    {
        _output.WriteLine("════════════════════════════════════════════════════════════");
        _output.WriteLine("=== Sample 9 재부팅 휘발성 테스트 초기화 (재부팅 후 로그) ===");
        _output.WriteLine("════════════════════════════════════════════════════════════\n");
        
        _output.WriteLine("📅 재부팅 정보:");
        _output.WriteLine($"   - 종료 시점: {_deviceShutdownTime:yyyy-MM-dd HH:mm:ss}");
        _output.WriteLine($"   - 시작 시점: {_deviceStartupTime:yyyy-MM-dd HH:mm:ss}");
        var shutdownDuration = _deviceStartupTime - _deviceShutdownTime;
        _output.WriteLine($"   - 종료 기간: {shutdownDuration.TotalHours:F1}시간\n");
    ...
    public async Task Should_Measure_DetectionRate_AfterReboot_AllApps()
    {
        ...
        // 세션별 앱 분포 분석
        var sessionsByApp = result.Sessions
        ...
        _output.WriteLine("\n📋 세션 탐지 상세 (앱별):");
        foreach (var app in sessionsByApp)
        {
            var appName = app.Package switch
            {
            ...
            _output.WriteLine($"   - {appName}: {app.Count}개 세션 탐지");
        }
        ...
        var detectionRate = ExpectedTotalCaptures > 0 
            ...
            : 0;
        _output.WriteLine($"촬영 탐지율: {detectionRate:F1}%");
        
        var sessionDetectionRate = ExpectedTotalSessions > 0
            ? (double)result.Sessions.Count / ExpectedTotalSessions * 100
            : 0;
        _output.WriteLine($"세션 탐지율: {sessionDetectionRate:F1}% ({result.Sessions.Count}/{ExpectedTotalSessions})\n");
        ...
        {
            _output.WriteLine("   ⚠️  usagestats.log 완전 휘발 → 세션 구분 불가");
        }
        else
        {
            _output.WriteLine($"   ✅ usagestats.log 일부 보존 ({usagestatsEventCount}개 이벤트)");
            _output.WriteLine($"      재부팅 후 {(_startTime - _deviceStartupTime).TotalHours:F1}시간 후 촬영");
        }
        
        if (mediaCameraEventCount == 0)
        {
            _output.WriteLine("   ⚠️  media_camera.log 완전 휘발 → 촬영 탐지 불가");
        ...
        _output.WriteLine("\n📝 선행 연구 검증 관점:");
        _output.WriteLine("   - usagestats.log: 재부팅 후에도 일부 보존 (앱별 차이 존재)");
        _output.WriteLine("   - media_camera.log: 메모리 기반 → 재부팅 시 완전 휘발");
        _output.WriteLine("   - 세션 탐지: usagestats만으로 부분적 가능 (앱별 차이)");
        _output.WriteLine("   - 촬영 탐지: media_camera 필수 → 재부팅 시 불가능");
        _output.WriteLine("════════════════════════════════════════════════════════════\n");
    ...
    {
        _output.WriteLine("════════════════════════════════════════════════════════════");
        _output.WriteLine("=== 기본 카메라 세션 상세 분석 (S9-2, 재부팅 후) ===");
        _output.WriteLine("════════════════════════════════════════════════════════════\n");
        ...
        AnalyzeDefaultCameraSession(
            ...
            _parsedEventsReboot!,
            "S9-2 (기본 카메라 촬영)",
            new DateTime(2025, 10, 17, 16, 42, 20),
            new DateTime(2025, 10, 17, 16, 42, 31),
            new DateTime(2025, 10, 17, 16, 42, 26),
            true,
            Standard);
    ...
    {
        _output.WriteLine("════════════════════════════════════════════════════════════");
        _output.WriteLine("=== 카카오톡 세션 상세 분석 (S9-3, S9-4, 재부팅 후) ===");
        _output.WriteLine("════════════════════════════════════════════════════════════\n");
        ...
        AnalyzeKakaoSession(
            ...
            _parsedEventsReboot!,
            "S9-3 (사용만)",
            new DateTime(2025, 10, 17, 16, 43, 18),
            new DateTime(2025, 10, 17, 16, 43, 23),
            null,
            false,
            Standard);
        ...
        AnalyzeKakaoSession(
            ...
            _parsedEventsReboot!,
            "S9-4 (촬영)",
            new DateTime(2025, 10, 17, 16, 45, 44),
            new DateTime(2025, 10, 17, 16, 45, 53),
            new DateTime(2025, 10, 17, 16, 45, 49),
            true,
            Standard);
    ...
    public void Should_Analyze_NonCaptureSession_Scores_AfterReboot()
    {
        ...
        {
            new { Name = "S9-1 (기본 카메라 사용만)", Start = new DateTime(2025, 10, 17, 16, 40, 58), End = new DateTime(2025, 10, 17, 16, 41, 4) },
            new { Name = "S9-3 (카카오톡 사용만)", Start = new DateTime(2025, 10, 17, 16, 43, 18), End = new DateTime(2025, 10, 17, 16, 43, 23) },
            new { Name = "S9-5 (텔레그램 사용만)", Start = new DateTime(2025, 10, 17, 16, 46, 39), End = new DateTime(2025, 10, 17, 16, 46, 52) },
            new { Name = "S9-7 (무음 카메라 사용만)", Start = new DateTime(2025, 10, 17, 16, 51, 23), End = new DateTime(2025, 10, 17, 16, 51, 29) }
        };
    ...
    public async Task Generate_GroundTruth_Document_RebootVolatility()
    {
        ...
        var sampleInfo = new ArtifactWeights.SampleInfo(
            SampleNumber: 9,
            SampleName: "9차 샘플 (재부팅 휘발성)",
            TestDate: new DateTime(2025, 10, 17),
            TimeRange: (_startTime, _endTime),
            Description: "기본 카메라, 카카오톡, 텔레그램, 무음 카메라 사용 (총 4회 촬영) - 재부팅 약 6시간 후 로그"
        );
        ...
        _output.WriteLine($"🔄 재부팅 정보: 종료 {_deviceShutdownTime:yyyy-MM-dd HH:mm:ss}, 시작 {_deviceStartupTime:yyyy-MM-dd HH:mm:ss}");
        _output.WriteLine("");
        ...
        var outputPath = Path.Combine(docDir, "Sample9_Reboot_Ground_Truth.md");
        ...
        gtDocument.Should().Contain("# Sample 9", "헤더가 있어야 함");
        ...
        var shutdownDuration = _deviceStartupTime - _deviceShutdownTime;
        _output.WriteLine($"   - 특징: 재부팅 약 {shutdownDuration.TotalHours:F1}시간 종료 후");
        _output.WriteLine("════════════════════════════════════════════════════════════");
    }
    ...
    private IAnalysisOrchestrator CreateOrchestratorWithYamlConfig()
    {
        ...
        var logger = loggerFactory.CreateLogger<Sample9RebootVolatilityTests>();
